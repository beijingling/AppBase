...
import com.ling.commonlib.basebean.BaseResponse;
            ...
            public Publisher<T> apply(Flowable<T> response)
            {
                return response.flatMap(new Function<T, Publisher<T>>()
                {
                    ...
                    public Publisher<T> apply(T t) throws Exception
                    {
                        ...
                        {
                            return Flowable.error(new AppException(BaseApplication.Companion.getAppContext().getString(R.string.server_excception), "-1"));
                        }
                    }
                });
            }
            ...
            public Flowable<T> apply(Flowable<BaseResponse<T>> httpResponseFlowable)
            {
                return httpResponseFlowable.flatMap(new Function<BaseResponse<T>, Flowable<T>>()
                {
                    ...
                    public Flowable<T> apply(BaseResponse<T> result)
                    {
                        ...
                        {
                            return Flowable.error(new AppException(BaseApplication.Companion.getAppContext().getString(R.string.server_excception), result.getCode()));
                        }
                    }
